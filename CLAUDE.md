...
## Item Statuses
`idea` → `option` → `shortlisted` → `confirmed` → `reserved` — this is the `status` column's full value set (DB CHECK constraint enforces exactly these; matches `ITEM_STATUSES` in `src/config/constants.js`). Completion is tracked separately via the `is_done` boolean (plus `done_by`/`done_at`), not as a `status` value — there is no `"done"` status.
