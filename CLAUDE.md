...
- `trip_items` — `base_id` and `day_id` independently nullable; `is_anchor` boolean; `time_start`/`time_end` local HH:MM strings; `cost_low`/`cost_high` USD numeric; `address` free-text nullable (renders as a map-pin icon; link searches `"<item title>, <address>"` so it resolves to the actual business listing, not just a bare pin — opens per the user's "Preferred maps app" setting: Apple Maps or Google Maps always, or Automatic which defaults to Apple Maps on Apple platforms since Apple has no user-facing default-maps-app setting on iOS/macOS, in the US or EU, to detect or respect); soft delete via `deleted_at`
