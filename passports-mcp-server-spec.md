...
(Second implementation note, same area: both documents must also be served at their **path-aware** locations — `/.well-known/oauth-protected-resource/api/mcp` — which is where RFC 9728 tells a client to look first for a resource that lives at `/api/mcp`. Until this was added, the SPA catch-all in `netlify.toml` answered those URLs with `index.html` and a `200`. That is the worst possible shape of wrong: a client expecting JSON gets a success status and HTML, throws while parsing, and aborts the sign-in flow — leaving no trace in our function logs at all, because it never reached a function. The same reasoning says any `/.well-known/` document we do not implement should return a real 404 rather than HTML — a 404 lets a client fall back cleanly, a 200 does not — but note that this is a principle, not a blanket rule in `netlify.toml`. Only `openid-configuration` is routed that way today. A catch-all `/.well-known/*` 404 was deliberately **not** added, because that path is also where Let's Encrypt serves `acme-challenge` during certificate renewal. New discovery documents that clients turn out to probe should get their own surgical rule.)

(Third note, correcting the second — this one cost a working "connect" step, so it is worth reading before touching these rules again. The **authorization-server** document must NOT be served under a path suffix, even though the protected-resource one must. The two RFCs look symmetric and are not. RFC 9728 keys the protected-resource document to a *resource* identifier, and `https://host/api/mcp` genuinely is our resource, so the `/api/mcp` suffix is correct there. RFC 8414 keys its document to an *issuer* identifier, and asks the client to check that the returned `issuer` field is identical to the issuer the URL asked about; a request to `/.well-known/oauth-authorization-server/api/mcp` is asking about an issuer of `https://host/api/mcp`, which is not us — our issuer is the bare origin. On that mismatch the spec says the client MUST NOT use the document. Serving it at the suffix therefore turned a clean fallback into a hard stop: before, that URL returned HTML and the client moved on to the suffix-less URL that matches; after, it returned a well-formed document the client was required to reject. Both suffixed paths now return 404. The general lesson is the one this whole section keeps re-learning from the other direction: a parseable 200 carrying the wrong answer is more dangerous than a 404, because it defeats the fallback that a 404 would have triggered.)
