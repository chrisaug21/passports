...
- Fields: `tripId`, `title`, `itemType`, optionally `baseId`, `dayId`, `mealSlot`/`activityType`/`transportMode` etc., `notes`, `url`. Reuses the shape of `createDetailedTripItem` in [`src/services/trips-service.js`](src/services/trips-service.js) — same validation (item_type/meal_slot/etc enums).
